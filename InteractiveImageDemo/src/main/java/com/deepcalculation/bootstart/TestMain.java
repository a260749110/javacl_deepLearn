package com.deepcalculation.bootstart;
			...
			new Thread() {
				public void run() {
					try {
						...
						CLImage2D imageOut = context.createImage2D(CLMem.Usage.InputOutput, imageIn.getFormat(), width,
								height);

						float[] f = { 0.2f, 0.5f, 0.8f };
						FloatBuffer buff = FloatBuffer.wrap(f);

						CLBuffer<Float> cldouble = context.createFloatBuffer(CLMem.Usage.InputOutput, buff, true);
						CLBuffer<Float> cldoubleot = context.createFloatBuffer(CLMem.Usage.InputOutput, width);
						...
						for (CLKernel kernel : kernels) {
							...
							try {
								...
								kernel.setArgs(imageIn, imageOut, cldouble, cldoubleot);
						...
						BufferedImage bufferedImageIn = imageOut.read(queue);

						Pointer<Float> buffer = cldoubleot.read(queue);
						List<Float> fl = buffer.asList();
						for (int i = 0; i < fl.size(); i++) {
							System.err.println("fl" + i + ":" + fl.get(i));
						}
						imageIn.release();
						imageOut.release();
						for (CLKernel kernel : kernels)
	...
	public TestMain() {
		...
		examplesCombo.addItem("Examples...");
		{
			...
			for (Example example : new Example[] {
					// "Blur",
					new Example("test", "test"), new Example("Blur", "Blur"), new Example("Convolution", "Convolution"),
					new Example("Sobel Operator", "SobelFilter"), new Example("Desaturate Colors", "DesaturateColors"),
					new Example("Richardson-Lucy Deconvolution", "RichardsonLucyDeconvolution"),
					new Example("Luminance Threshold", "LuminanceThreshold"),
					new Example("Naive Denoising", "NaiveDenoising"), new Example("Identity", "Identity"),
		...
		add("Center",
				imgSrcSplitPane = new JSplitPane(JSplitPane.VERTICAL_SPLIT,
						imgsSplitPane = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, withTitle("Input", origImgScroll),
		...
		origImgLab.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				chooseImage();
			}
		});
		resultImgLab.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				saveResult();
			}
		});
